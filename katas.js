...
const psicologo = () => {console.log('Com estàs?')};
...
const pregunta = (someValue) => {console.log(`T'agrada ${someValue}?`)};
...
const pregunta2 = (someValue) => {return `T'agrada ${someValue}`};

console.log(pregunta2("la pizza") + " amb piña?");


// exercici 5: defineix una funció metresCubicsALitres() 
//que passi metres cúbics a litres i retorni el resultat
// i una altra mostrarSolució() que usi la primera per 
//mostrar per la consola "X metres cúbics són Y litres"

var m3 = 8;

// TODO: aquí les definicions de metresCubicsALitres() i mostrarSolució() 
let metresCubicsALitres = (vol) => {return m3 * 1000};

let mostrarSolucio = (vol, someFunc) => {
    console.log(`${vol} metres cùbics són ${someFunc(vol)} litres`);
};

mostrarSolucio(m3,metresCubicsALitres);


// exercici 6: defineix dues funcions 
//afegirDiners() i gastarDiners() que sumi/resti a 
//una variable externa el valor
// indicat al paràmetre i mostri a la consola la quantitat actual de diners

var diners = 0;

// TODO: definició de afegirDiners() i gastarDiners() 
const afegirDiners = (someValue) => {
    diners += someValue;
};

const gastarDiners = (someValue) => {
    diners -= someValue;
};
...
// exercici 7: fes un diàleg que et demani la talla de samarreta que 
//vols comprar (S, M, L, XL) i et digui per la consola
// quina talla has sel·leccionat i t'avisi si sel·lecciones suna talla que no és vàlida



var talla = prompt("Esculli la talla: S, M, L, XL");  // TODO: editar el missatge del prompt()
...
let result;
switch(talla){
    case "S": result = `has seleccionat ${talla}`
        break;
    case "M": result = `has seleccionat ${talla}`
        break; 
    case "L": result = `has seleccionat ${talla}`
        break;
    case "XL": result = `has seleccionat ${talla}`
        break;  
    default: result = `tens que seleccionar una talla vàlida`
    break;   
}       
 console.log(result);

// exercici 8: defineix una funció amb modes que 
//pugui calcular l'area o el perímetre d'un cercle i et faci console.log()
